...
export function buildChecks(project, inventory) {
  ...
  for (const element of project.elements) {
    ...
    if (element.operation === 'raise') {
      ...
      if (element.operation === 'inlay' && element.inlayHeight > 0 && Math.abs(element.inlayHeight / layerHeight - Math.round(element.inlayHeight / layerHeight)) > .04) checks.push({ level: 'warn', elementId: element.id, title: `${element.name} flat-color top falls between layers`, message: 'Snap the flat-color top height to the selected layer height.' });
